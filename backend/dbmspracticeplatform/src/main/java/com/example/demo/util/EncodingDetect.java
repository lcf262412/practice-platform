...
class BytesEncodingDetect extends Encoding {
    ...
    public int detectEncoding(URL testurl) {
        ...
        try {
            ...
            log.error("Error loading or using URL " + e.toString());
            guess = -1;
        } finally {
    ...
    public int detectEncoding(byte[] rawtext) {
        ...
        for (index = 0; index < TOTALTYPES; index++) {
            if (debug)
                log.error("Encoding " + nicename[index] + " score " + scores[index]);
    ...
    int gb2312_probability(byte[] rawtext) {
        ...
        BigDecimal rangevalBigDecimal = new BigDecimal(rangeval);
        BigDecimal freqvalBigDecimal = new BigDecimal(freqval);
        return rangevalBigDecimal.add(freqvalBigDecimal).intValue();
    }
    ...
    int gbk_probability(byte[] rawtext) {
        ...
        for (i = 0; i < rawtextlen - 1; i++) {
            if (rawtext[i] >= 0) {
                ...
                dbchars++;
                boolean isOriginalGBRange = (byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF7 && // Original GB range
                        (byte) 0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE;
                boolean isExtendedGBRangePart1 = (byte) 0x80 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE;
                boolean isExtendedGBRangePart2 = (byte) 0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7E;
                boolean isExtendedGBRangePart = isExtendedGBRangePart1 || isExtendedGBRangePart2;
                boolean isExtendedGBRange = (byte) 0x81 <= rawtext[i] && rawtext[i] <= (byte) 0xFE && isExtendedGBRangePart; // Extended GB range
                if (isOriginalGBRange) {
                    gbchars++;
                    totalfreq += 500;
                    row = rawtext[i] + 256 - 0xA1;
                    column = rawtext[i + 1] + 256 - 0xA1;
        ...
        BigDecimal rangevalBigDecimal = new BigDecimal(rangeval);
        BigDecimal freqvalBigDecimal = new BigDecimal(freqval);
        return rangevalBigDecimal.add(freqvalBigDecimal).intValue() - 1;
    }
    ...
    int gb18030_probability(byte[] rawtext) {
        ...
        for (i = 0; i < rawtextlen - 1; i++) {
            if (rawtext[i] >= 0) {
                ...
                dbchars++;
                boolean OriginalGBRangePart1 = (byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF7;
    ...
    int big5_probability(byte[] rawtext) {
        ...
        for (i = 0; i < rawtextlen - 1; i++) {
            if (rawtext[i] >= 0) {
                ...
                dbchars++;
                boolean isBig5RangePart1 = (byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF9;
                boolean isBig5RangePart2 = ((byte) 0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7E) || ((byte) 0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE);
                boolean isBig5Range = isBig5RangePart1 && isBig5RangePart2;
                if (isBig5Range) {
                    bfchars++;
                    totalfreq += 500;
                    row = rawtext[i] + 256 - 0xA1;
    ...
    int big5plus_probability(byte[] rawtext) {
        ...
        for (i = 0; i < rawtextlen - 1; i++) {
            if (rawtext[i] >= 128) {
                ...
                dbchars++;
                boolean isOriginalBig5RangePart1 = 0xA1 <= rawtext[i] && rawtext[i] <= 0xF9;
                boolean isOriginalBig5RangePart2 = (0x40 <= rawtext[i + 1] && rawtext[i + 1] <= 0x7E) || (0xA1 <= rawtext[i + 1] && rawtext[i + 1] <= 0xFE);
                boolean isOriginalBig5Range = isOriginalBig5RangePart1 && isOriginalBig5RangePart2; // Original Big5 range
                boolean isExtendBig5RangePart1 = 0x81 <= rawtext[i] && rawtext[i] <= 0xFE;
                boolean isExtendBig5RangePart2 = (0x40 <= rawtext[i + 1] && rawtext[i + 1] <= 0x7E) || (0x80 <= rawtext[i + 1] && rawtext[i + 1] <= 0xFE);
                boolean isExtendBig5Range = isExtendBig5RangePart1 && isExtendBig5RangePart2; // Extended Big5 range
                if (isOriginalBig5Range) {
                    bfchars++;
                    totalfreq += 500;
                    row = rawtext[i] - 0xA1;
                    if (0x40 <= rawtext[i + 1] && rawtext[i + 1] <= 0x7E) {
    ...
    int utf8_probability(byte[] rawtext) {
        ...
        rawtextlen = rawtext.length;
        for (i = 0; i < rawtextlen; i++) {
            boolean isTwoBytePart1 = -64 <= rawtext[i] && rawtext[i] <= -33;
        	boolean isTwoBytePart2 = i + 1 < rawtextlen && -128 <= rawtext[i + 1] && rawtext[i + 1] <= -65;
    ...
    int cp949_probability(byte[] rawtext) {
        ...
        for (i = 0; i < rawtextlen - 1; i++) {
            if (rawtext[i] >= 0) {
                ...
                dbchars++;
                boolean isCP949RangePart1 = (byte) 0x81 <= rawtext[i] && rawtext[i] <= (byte) 0xFE;
                boolean isCP949RangePart2Part1 = (byte) 0x41 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x5A;
                boolean isCP949RangePart2Part2 = (byte) 0x61 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7A;
                boolean isCP949RangePart2Part3 = (byte) 0x81 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFE;
                boolean isCP949RangePart2 = isCP949RangePart2Part1 || isCP949RangePart2Part2 || isCP949RangePart2Part3;
                boolean isCP949Range = isCP949RangePart1 && isCP949RangePart2;
                if (isCP949Range) {
                    krchars++;
                    totalfreq += 500;
                    if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xFE && (byte) 0xA1 <= rawtext[i + 1]
    ...
    int sjis_probability(byte[] rawtext) {
        ...
        for (i = 0; i < rawtextlen - 1; i++) {
            if (rawtext[i] >= 0) {
                ...
                dbchars++;
                boolean isSjisRangePart1 = ((byte) 0x81 <= rawtext[i] && rawtext[i] <= (byte) 0x9F) || ((byte) 0xE0 <= rawtext[i] && rawtext[i] <= (byte) 0xEF);
                boolean isSjisRangePart2 = ((byte) 0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7E) || ((byte) 0x80 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0xFC);
                boolean isSjisRange = i + 1 < rawtext.length && isSjisRangePart1 && isSjisRangePart2;
                if (isSjisRange) {
                    jpchars++;
                    totalfreq += 500;
                    row = rawtext[i] + 256;
